...
export class Storage {
        ...
        try {
            ...
            if (data) {
                ...
                return {
                    ...
                    unlockedThemes: Array.isArray(parsed.unlockedThemes) ? parsed.unlockedThemes : defaultState.unlockedThemes,
                    currentThemeId: typeof parsed.currentThemeId === 'string' ? parsed.currentThemeId : defaultState.currentThemeId
                };
            }
        } catch (e) {
    ...
    static addPoints(amount: number): void {
        const state = this.load();
        state.points += amount;
        this.save(state);
    }

    static spendPoints(amount: number): boolean {
        const state = this.load();
    ...
    static getUnlockedThemes(): string[] {
        return this.load().unlockedThemes;
    }

    static unlockTheme(themeId: string): void {
        const state = this.load();
    ...
    static getCurrentThemeId(): string {
        return this.load().currentThemeId;
    }
